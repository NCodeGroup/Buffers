...
/// </para>
/// <para>
/// The buffer writer tracks the current write position via <see cref="WrittenCount"/> and provides
/// <see cref="WrittenMemory"/> and <see cref="WrittenSpan"/> to access only the written portion of the buffer.
/// </para>
/// </remarks>
/// <example>
/// <code>
...
///
/// // Access written data
/// ReadOnlyMemory&lt;byte&gt; written = writer.WrittenMemory; // Contains [0x01, 0x02]
/// </code>
/// </example>
[PublicAPI]
public class FixedMemoryBufferWriter<T> : IBufferWriter<T>
{
    ...
    /// <remarks>
    /// The returned memory represents only the portion of the buffer that has been written to,
    /// from index 0 to <see cref="WrittenCount"/> - 1. This memory can be passed to async methods
    ...
    /// <remarks>
    /// The returned span represents only the portion of the buffer that has been written to,
    /// from index 0 to <see cref="WrittenCount"/> - 1.
    /// </remarks>
    ...
    /// </exception>
    /// <remarks>
    /// Call this method after writing data to the memory/span returned by <see cref="GetMemory"/> or <see cref="GetSpan"/>
    /// to update the write position. The <paramref name="count"/> should be the exact number of elements written.
    /// </remarks>
    ...
    /// <remarks>
    /// <para>
    /// After writing to the returned memory, you must call <see cref="Advance"/> to update the write position.
    /// </para>
    /// <para>
    /// The returned memory may be larger than <paramref name="sizeHint"/>; it represents all remaining free space in the buffer.
    /// This memory can be passed to async methods or stored for later use.
    /// </para>
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <remarks>
    /// <para>
    /// After writing to the returned span, you must call <see cref="Advance"/> to update the write position.
    /// </para>
    /// <para>
    /// The returned span may be larger than <paramref name="sizeHint"/>; it represents all remaining free space in the buffer.
    /// </para>
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
